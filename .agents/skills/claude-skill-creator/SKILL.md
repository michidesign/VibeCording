...
description: Guide for creating effective Codex skills with proper YAML frontmatter, directory structure, and best practices. Use when creating new skills, updating existing skills, or learning about skill development.
---

# Codex Skill Creator Guide

This guide helps you create well-structured, effective skills for Codex that extend capabilities with specialized knowledge, workflows, or tool integrations.
...
Skills extend Codex's capabilities through organized folders containing instructions and resources. They are **model-invoked**—Codex autonomously decides when to use them based on request context, unlike slash commands which require explicit user activation.
...
2. **Project Skills**: `.Codex/skills/skill-name/`
...
Specific guidance for Codex on how to use this skill:
...
Use `allowed-tools` to limit Codex's capabilities when the skill is active:
...
This restricts Codex to only specified tools, useful for:
...
Users and Codex learn best from examples:
...
2. **Test Instructions**: Does Codex follow the workflow correctly?
...
Codex will only load these files when needed, saving context tokens.

### 9. Provide Utility Scripts for Complex Tasks

**Checklisted Workflows**: Include ordered steps Codex can track progress through

**Pre-written Scripts**: For deterministic operations, provide ready-to-use scripts rather than having Codex generate them
- More reliable
- Saves tokens
- Faster execution
...
### Issue: Codex Doesn't Use the Skill
...
5. **Directory structure is wrong**
   - ✅ Fix: Ensure path is `~/.Codex/skills/skill-name/SKILL.md`
...
3. **Missing AI Assistant Instructions section**
   - ✅ Fix: Add explicit guidance for Codex

## Sharing Skills with Teams

### For Project Skills

1. Create skill in `.Codex/skills/skill-name/`
2. Commit to git:
   ```bash
...
- [ ] Directory created: `~/.Codex/skills/skill-name/` or `.Codex/skills/skill-name/`
...
7. **Write for Autonomous Use**: Codex should be able to use the skill without asking clarifying questions